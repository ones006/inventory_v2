class Tracker < ActiveRecord::Base
  ...
  def log_with_self(entry, quantity)
    ...
    tracker = company.trackers.new(:item_id => item_id,
    ...
    if quantity >= new_available_quantity
      ...
      Tracker.update_all("closed = 1", "stock_entry_id = #{stock_entry_id} AND item_id = #{item_id} AND closed = 0")
    else
  ...
  def log_with_new_reference(entry, quantity, ref_entry)
    tracker = ref_entry.company.trackers.new(:item_id => ref_entry.item.id,
    ...
    if quantity >= ref_entry.quantity
      ...
      Tracker.update_all("closed = 1", "stock_entry_id = #{ref_entry.id} AND item_id = #{ref_entry.item.id} AND closed = 0")
    else
      tracker.closed            = false
      tracker.available_stock   = ref_entry.quantity
      tracker.consumed_stock    = quantity
